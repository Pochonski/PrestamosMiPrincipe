...
create index if not exists idx_cuotas_prestamo_estado on public.cuotas(prestamo_id, estado);
create index if not exists idx_cuotas_estado_fecha on public.cuotas(estado, fecha);
...
create index if not exists idx_cobros_prestamo on public.cobros(prestamo_id);
