...
export const Sidebar = memo(function Sidebar({ open, page, onNavigate, onClose, orgName, userName, rol }) {
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          'glass fixed bottom-3 left-3 top-3 z-50 flex flex-col rounded-modal',
          'transition-transform duration-300 ease-out',
          'w-[var(--sidebar-w)] lg:translate-x-0',
          open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0',
        )}
      >
        <div className="border-b border-white/50 px-4 pb-3 pt-4 dark:border-white/10">
          ...
          {(orgName || rol) && (
            <div className="glass-subtle mt-2.5 flex items-center gap-2 rounded-input px-2.5 py-1.5">
        ...
        <nav className="flex-1 overflow-y-auto px-2.5 py-3 scrollbar-thin">
          <p className="label-micro px-3 pb-1.5">Principal</p>
          ...
          <p className="label-micro mt-5 px-3 pb-1.5">Acciones</p>
          <ul className="space-y-0.5">
            {acciones.map((item) => (
              <NavLink
                key={item.id}
        ...
        {(userName || rol) && (
          <div className="border-t border-white/50 px-4 py-2.5 dark:border-white/10">
  ...
  return (
    <li>
      <button
        ...
        className={clsx(
          'flex min-h-[44px] w-full items-center gap-2.5 rounded-input border border-transparent px-2.5 py-2 font-display text-sm font-semibold transition-all duration-150 relative glass-glare',
      ...
      >
        ...
        <span
          aria-hidden="true"
          className={clsx(
            'flex h-7 w-7 shrink-0 items-center justify-center rounded-input transition-colors',
            active
